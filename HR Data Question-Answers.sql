...
select round(avg(DATEDIFF(termdate,hire_date)/365)) as avg_length_employment from hr 
...
select 
	year,hires,termination,hires-termination as net_change,round((hires-termination)/hires*100,2) as net_change_percent
    from(
    select year(hire_date) as year,count(*) as hires,sum(case when termdate <> '0000-00-00' and termdate<= curdate() then 1 else 0 end) as termination
...
group by department;select department,count(*) from hr 
group by department
